...
contract TestSetToken is SystemFixture {
    ...
    function testComponents() external {
        ...
        assertEq(
            setToken.getDefaultPositionRealUnit(address(dai)),
            toDAIUnits(10000)
        );
        assertEq(
            setToken.getDefaultPositionRealUnit(address(wbtc)),
            toWBTCUnits(5) / 10
        );
    }
    ...
    function testEditPositionMultiplier() external {
        ...
        int256 expectedRealUnit = PreciseUnitMath.preciseMul(
            toDAIUnits(10000),
            multiplier
        );
        assertEq(
    ...
    function testAddModule() external {
        ...
        assertEq(
            uint256(moduleState),
            uint256(ISetToken.ModuleState.INITIALIZED)
        );
    }
    ...
    function testRemoveModule() external {
        ...
        ISetToken.ModuleState moduleState = setToken.moduleStates(
            newModuleAddress
        );
        assertEq(uint256(moduleState), uint256(ISetToken.ModuleState.PENDING));
