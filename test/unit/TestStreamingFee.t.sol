...
contract TestStreamingFee is SystemFixture {
    ...
    function getStreamingFee(
        uint256 previousAccrueTimestamp,
        uint256 recentAccrueTimestamp,
        uint256 streamingFee
    ) internal view returns (uint256) {
        (, , uint256 streamingFeePercentage, ) = streamingFeeModule.feeStates(
            setToken
        );
        uint256 accrualRate;
        if (streamingFee != 0) {
    ...
    function getStreamingFeeInflationAmount(
    ...
    function getPostFeePositionUnits(
        int256[] memory preFeeUnits,
        uint256 inflationPercent
    ) internal view returns (int256[] memory newUnits) {
            ...
            if (preFeeUnits[i] >= 0) {
                newUnits[i] = (
                    PreciseUnitMath.preciseMul(
                        preFeeUnits[i],
                        int256(PRECISE_UNIT - inflationPercent)
                    )
                );
            } else {
                newUnits[i] = int256(
                    PreciseUnitMath.preciseMulCeil(
                        uint256(preFeeUnits[i]),
                        PRECISE_UNIT - inflationPercent
                    )
                );
            }
        }
    ...
    function testAccrueFeeMintsCorrectAmount() external {
        ...
        uint256 expectedFeeInflation = getStreamingFee(
            previousAccrueTimestamp,
            recentAccrueTimestamp,
            0
        );
        uint256 feeInflation = getStreamingFeeInflationAmount(
            expectedFeeInflation,
            totalSupply
        );
        uint256 protocolFeeAmount = (feeInflation * protocolFee) / 1 ether;
        assertEq(
            setToken.balanceOf(streamingFeeRecepient),
    ...
    function testLastStreamingFeeTimestampSetCorrectly() external {
        ...
        streamingFeeModule.accrueFee(setToken);
        (, , , uint256 lastStreamingFeeTimestamp) = streamingFeeModule
            .feeStates(setToken);
        assertEq(lastStreamingFeeTimestamp, executionTimestamp);
    }

    function testUpdatesPositionMultiplierCorrectly() external {
        (, , , uint256 previousAccrueTimestamp) = streamingFeeModule.feeStates(
            setToken
        );
        uint256 recentAccrueTimestamp = block.timestamp + ONE_YEAR_IN_SECONDS;
        uint256 expectedFeeInflation = getStreamingFee(
    ...
    function testUpdatesPositionCorrectly() external {
        ...
        int256[] memory expectedNewUnits = getPostFeePositionUnits(
            preFeeUnits,
            expectedFeeInflation
        );
    ...
    function testProtocolFeeZeroMintsNoSets() external {
        vm.prank(deployer);
        Controller(address(controller)).editFee(
            address(streamingFeeModule),
            0,
            0
        );
