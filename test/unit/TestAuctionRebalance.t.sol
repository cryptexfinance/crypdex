...
contract TestAuctionRebalance is AuctionFixture {
    ...
    event RebalanceStarted(
        ...
        AuctionRebalanceModuleV1.AuctionExecutionParams[] auctionParameters
    );
    event BidExecuted(
        ISetToken indexed setToken,
    ...
    function testSetsAuctionParamsCorrectly() external {
        ...
        for (uint256 i = 0; i < defaultOldComponentsAuctionParams.length; i++) {
            ...
            ) = auctionRebalanceModuleV1.executionInfo(
                    setToken,
                    IERC20(aggregateComponents[i])
                );
            assertEq(
                targetUnit,
                defaultOldComponentsAuctionParams[i].targetUnit
            );
            assertEq(
                priceAdapterName,
                defaultOldComponentsAuctionParams[i].priceAdapterName
            );
            assertEq(
                priceAdapterConfigData,
                defaultOldComponentsAuctionParams[i].priceAdapterConfigData
            );
        }
    }
    ...
    function testIsRebalanceDurationElapsed() external {
        ...
        assertTrue(
            auctionRebalanceModuleV1.isRebalanceDurationElapsed(setToken)
        );
    }
    ...
    function testGetRebalanceComponents() external {
        ...
        address[] memory rebalanceComponents = auctionRebalanceModuleV1
            .getRebalanceComponents(setToken);
    ...
    function testGetAuctionSizeAndDirectionSellAuction() external {
        ...
        ) = auctionRebalanceModuleV1.getAuctionSizeAndDirection(
                setToken,
                IERC20(address(dai))
            );
        uint256 totalSupply = setToken.totalSupply();
        uint256 expectedDaiSize = PreciseUnitMath.preciseMul(
            uint256(toWETHUnits(900)),
            totalSupply
        );
    ...
    function testGetAuctionSizeAndDirectionBuyAuction() external {
        ...
        uint256 totalSupply = setToken.totalSupply();
        uint256 expectedWbtcSize = PreciseUnitMath.preciseMul(
            uint256(toWBTCUnits(1) / 10),
            totalSupply
        );
    ...
    function testGetQuoteAssetBalance() external {
        ...
        assertEq(
            auctionRebalanceModuleV1.getQuoteAssetBalance(setToken),
            uint256(toWETHUnits(5))
        );
    }
    ...
    function testGetBidPreview() external {
        ...
        AuctionRebalanceModuleV1.BidInfo
            memory bidInfo = auctionRebalanceModuleV1.getBidPreview(
                setToken,
                subjectComponent,
                subjectQuoteAsset,
    ...
    function testSetRaiseTargetPercentage() external {
        ...
        (, , , , uint256 newRaiseTargetPercentage) = auctionRebalanceModuleV1
            .rebalanceInfo(setToken);
    ...
    function testRaiseAssetTargets() external {
        AuctionRebalanceModuleV1.AuctionExecutionParams[]
            memory oldComponentsAuctionParams = new AuctionRebalanceModuleV1.AuctionExecutionParams[](
                3
            );
        oldComponentsAuctionParams[0] = AuctionRebalanceModuleV1
        ...
        oldComponentsAuctionParams[1] = AuctionRebalanceModuleV1
            .AuctionExecutionParams({
                targetUnit: uint256(toWBTCUnits(54) / int256(100)),
                priceAdapterName: CONSTANT_PRICE_ADAPTER,
                priceAdapterConfigData: defaultWbtcData
            });
        oldComponentsAuctionParams[2] = AuctionRebalanceModuleV1
            .AuctionExecutionParams({
        ...
        placeBid(
            ...
            dai,
            IERC20(address(weth)),
            uint256(toDAIUnits(900)),
            uint256(toWETHUnits(45)) / 100,
            true
        );
        placeBid(
            setToken,
            wbtc,
            IERC20(address(weth)),
            uint256(toWBTCUnits(4)) / 100,
            uint256(toWETHUnits(58)) / 100,
            false
        );

        (, , , uint256 prePositionMultiplier, ) = auctionRebalanceModuleV1
            .rebalanceInfo(setToken);
        ...
        uint256 expectedPositionMultiplier = PreciseUnitMath.preciseDiv(
            prePositionMultiplier,
            PRECISE_UNIT + raiseTargetPercentage
        );
        ...
        (bool daiDirection, uint256 daiSize) = auctionRebalanceModuleV1
            .getAuctionSizeAndDirection(setToken, IERC20(address(dai)));
        (bool wbtcDirection, uint256 wbtcSize) = auctionRebalanceModuleV1
            .getAuctionSizeAndDirection(setToken, IERC20(address(wbtc)));
    ...
    function testIsQuoteAssetExcessOrAtTarget() external {
        ...
        assertTrue(
            auctionRebalanceModuleV1.isQuoteAssetExcessOrAtTarget(setToken)
        );
    }
    ...
    function testIsAllowedBidder() external {
        ...
        assertTrue(
            auctionRebalanceModuleV1.isAllowedBidder(setToken, newBidder)
        );
    }
    ...
    function testSetAnyoneBid() external {
        ...
        emit AnyoneBidUpdated(setToken, true);
    ...
    function testNewComponentAdded() external {
        uint256 usdcPerWethDecimalFactor = uint256(
            toWETHUnits(1) / toUSDCUnits(1)
        );
        uint256 usdcPerWethPrice = (5 ether * usdcPerWethDecimalFactor) / 10000;
        bytes memory usdcPerWethBytes = constantPriceAdapter.getEncodedData(
            usdcPerWethPrice
        );
        AuctionRebalanceModuleV1.AuctionExecutionParams
            memory indexUsdcAuctionExecutionParams = AuctionRebalanceModuleV1
                .AuctionExecutionParams({
        ...
        AuctionRebalanceModuleV1.AuctionExecutionParams[]
            memory newComponentsAuctionParams = new AuctionRebalanceModuleV1.AuctionExecutionParams[](
                1
            );
        ...
        placeBid(
            ...
            usdc,
            IERC20(address(weth)),
            uint256(toUSDCUnits(100)),
            uint256(toWETHUnits(5) / 100),
            false
        );
    ...
    function testUnlockEarly() external {
        ...
        emit LockedRebalanceEndedEarly(setToken);
    ...
    function testUnlockWhenDurationElapsed() external {
        ...
        (, , , , uint256 raiseTargetPercentageBefore) = auctionRebalanceModuleV1
            .rebalanceInfo(setToken);
        ...
        auctionRebalanceModuleV1.unlock(setToken);
        assertFalse(setToken.isLocked());
        (, , , , uint256 raiseTargetPercentageAfter) = auctionRebalanceModuleV1
            .rebalanceInfo(setToken);
    ...
    function testBidWithBoundedStepwiseLinearPriceAdapter() external {
        ...
        bytes memory wbtcLinearCurveParams = boundedStepwiseLinearPriceAdapter
            .getEncodedData(
        ...
        bytes memory wethLinearCurveParams = boundedStepwiseLinearPriceAdapter
            .getEncodedData(1 ether, 0, 1 hours, false, 1 ether, 1 ether);
        AuctionRebalanceModuleV1.AuctionExecutionParams[]
            memory oldComponentsAuctionParams = new AuctionRebalanceModuleV1.AuctionExecutionParams[](
                3
            );
    ...
    function testBidWithBoundedStepwiseExponentialPriceAdapter() external {
        bytes
            memory daiExponentialCurveParams = boundedStepwiseExponentialPriceAdapter
                .getEncodedData(
                    5 ether / 10000,
                    1 ether,
                    1 ether / 100000,
        ...
        uint256 wbtcPerWethDecimalFactor = 1 ether / uint256(toWBTCUnits(1));
        bytes
            memory wbtcExponentialCurveParams = boundedStepwiseExponentialPriceAdapter
                .getEncodedData(
        ...
        bytes
            memory wethExponentialCurveParams = boundedStepwiseExponentialPriceAdapter
                .getEncodedData(
                    1 ether,
                    1 ether,
                    1 ether / 10,
    ...
    function testBidWithBoundedStepwiseLogarithmicPriceAdapter() external {
        bytes
            memory daiLogarithmicCurveParams = boundedStepwiseLogarithmicPriceAdapter
                .getEncodedData(
                    5 ether / 10000,
                    1 ether,
                    1 ether / 100000,
        ...
        uint256 wbtcPerWethDecimalFactor = 1 ether / uint256(toWBTCUnits(1));
        bytes
            memory wbtcLogarithmicCurveParams = boundedStepwiseLogarithmicPriceAdapter
                .getEncodedData(
                    14.5 ether * wbtcPerWethDecimalFactor,
                    1 ether,
                    (1 ether / 10) * wbtcPerWethDecimalFactor,
        ...
        bytes
            memory wethLogarithmicCurveParams = boundedStepwiseLogarithmicPriceAdapter
                .getEncodedData(
                    1 ether,
                    1 ether,
                    1 ether / 10,
    ...
    function testIndexWithoutQuoteAsset() external {
        ...
        uint256 postSetTokenWeth = weth.balanceOf(
            address(setTokenWithoutQuote)
        );
        uint256 postSetTokenWbtc = wbtc.balanceOf(
            address(setTokenWithoutQuote)
        );
        ...
        assertEq(
            postSetTokenWbtc,
            preSetTokenWbtc + uint256(toWBTCUnits(1)) / 100
        );
    }
}
