...
contract TestBasicIssuance is SystemFixture {
    ...
    function testInitialize() external {
        ...
        basicIssuanceModule.initialize(
            singleSetToken,
            IManagerIssuanceHook(randomAddress)
        );
        assertTrue(
            singleSetToken.isInitializedModule(address(basicIssuanceModule))
        );
        vm.stopPrank();
    }
    ...
    function testIssuanceHooks() external {
        ...
        assertEq(
            address(basicIssuanceModule.managerIssuanceHook(singleSetToken)),
            randomAddress
        );
        vm.stopPrank();
    }
    ...
    function testIssueComponentsDeposited() external {
        ...
        uint256 expectedBalance = (issueQuantity * uint256(toWBTCUnits(2))) /
            uint256(toWETHUnits(1));
