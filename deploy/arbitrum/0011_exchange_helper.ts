...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  let basicIssuanceModuleAddress = (await deployments.get(
    "BasicIssuanceModule",
  )).address;
  ...
  let arFI = await deployments.get("ArFIIndexToken");
  ...
  let toApprove = [
    [[usdc, weth, aave, gmx, uni, pendle, Stg, Crv], paraSwapV6Address],
    [[aave, gmx, uni, pendle, Stg, Crv], basicIssuanceModuleAddress],
  ];
  let tx;
  ...
  console.log("adding basicIssuanceModuleAddress for arFI");
  ...
  console.log(tx);
  await tx.wait(2);
  console.log("added basicIssuanceModuleAddress for arFI");
