// npx hardhat run scripts/arbitrum/setupPriceAdapters.ts --network arbitrum
...
async function main() {
  ...
  const integrationRegistry = new IntegrationRegistry__factory(
    deployerSigner,
  ).attach(integrationRegistryDeployment.address);

  const constantPriceAdapter = await deployments.get("ConstantPriceAdapter");
  const boundedStepwiseExponentialPriceAdapter = await deployments.get(
    "BoundedStepwiseExponentialPriceAdapter",
  );
  const boundedStepwiseLinearPriceAdapter = await deployments.get(
    "BoundedStepwiseLinearPriceAdapter",
  );
  const boundedStepwiseLogarithmicPriceAdapter = await deployments.get(
