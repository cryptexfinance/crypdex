...
contract GeneralIndexModule is ModuleBase, ReentrancyGuard {
        ...
        virtual
    {
        ...
        require(
            (netReceiveAmount + protocolFee) < executionInfo[_setToken][_component].maxSize,
    ...
    */
    ...
    function _accrueProtocolFee(TradeInfo memory _tradeInfo) internal returns (uint256 protocolFee) {
        uint256 exchangedQuantity =  IERC20(_tradeInfo.receiveToken).balanceOf(
            address(_tradeInfo.setToken)) - _tradeInfo.preTradeReceiveTokenBalance;
        ...
        returns (uint256 netSendAmount, uint256 netReceiveAmount)
    {
        ...
        netSendAmount = _tradeInfo.preTradeSendTokenBalance- postTradeSendTokenBalance;
        netReceiveAmount = postTradeReceiveTokenBalance - _tradeInfo.preTradeReceiveTokenBalance;
    ...
    function _validateTradeParameters(ISetToken _setToken, IERC20 _component) internal view virtual {
        ...
        require(
            (componentInfo.lastTradeTimestamp + componentInfo.coolOffPeriod) <= block.timestamp,
